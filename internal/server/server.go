...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{"tier": s.limits.Tier, "upgrade_url": "https://stockyard.dev/relay/"})
	})
...
func (s *Server) listChannels(w http.ResponseWriter, r *http.Request) {
	wj(w, 200, map[string]any{"channels": oe(s.db.ListChannels())})
}
func (s *Server) createChannel(w http.ResponseWriter, r *http.Request) {
	var c store.Channel
	json.NewDecoder(r.Body).Decode(&c)
	if c.Name == "" || c.Slug == "" {
		we(w, 400, "name and slug required")
		return
	}
	c.Enabled = true
	s.db.CreateChannel(&c)
	wj(w, 201, s.db.GetChannel(c.ID))
}
func (s *Server) getChannel(w http.ResponseWriter, r *http.Request) {
	c := s.db.GetChannel(r.PathValue("id"))
	if c == nil {
		we(w, 404, "not found")
		return
	}
...
func (s *Server) deleteChannel(w http.ResponseWriter, r *http.Request) {
	s.db.DeleteChannel(r.PathValue("id"))
	wj(w, 200, map[string]string{"deleted": "ok"})
}
func (s *Server) listDeliveries(w http.ResponseWriter, r *http.Request) {
	wj(w, 200, map[string]any{"deliveries": oe(s.db.ListDeliveries(r.PathValue("id"), 50))})
}
func (s *Server) stats(w http.ResponseWriter, r *http.Request) { wj(w, 200, s.db.Stats()) }
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	st := s.db.Stats()
	wj(w, 200, map[string]any{"status": "ok", "service": "relay", "channels": st.Channels})
}
func (s *Server) receiveHook(w http.ResponseWriter, r *http.Request) {
	slug := strings.TrimPrefix(r.URL.Path, "/hook/")
	ch := s.db.GetBySlug(slug)
	if ch == nil {
		we(w, 404, "channel not found")
		return
	}
	if !ch.Enabled {
		we(w, 503, "channel disabled")
		return
	}
	body, _ := io.ReadAll(r.Body)
	hdrs := fmt.Sprintf("%v", r.Header)
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("%s: warning: could not parse config.json: %v", "relay", err)
		return
	}
	s.pCfg = cfg
	log.Printf("%s: loaded personalization from %s", "relay", path)
}
